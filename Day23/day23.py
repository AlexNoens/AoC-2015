def load_input(file):
    ...
    for each in f.readlines():
        ...
        if len(instructions) == 1:
            ...
            if inst_reg[0] == "jmp":
                lines.append((inst_reg[0].strip(), int(inst_reg[1].strip()), 0))
            else:
                lines.append((inst_reg[0].strip(), inst_reg[1].strip(), 0))
        else:
            inst_reg = instructions[0].split(" ")
            lines.append(
                (inst_reg[0].strip(), inst_reg[1].strip(), int(instructions[1].strip()))
            )
    return lines
...
# (instruction, r/offset, offset)
process = load_input("Day23/input.txt")
