        ...
        for arm in armor:
            for i in range(0,7):
                ...
                for j in range(0,7):
                    ...
                    Charecters["Player"] = Charecter(arm[1] + ring_1[2] + ring_2[2], stabber[1] + ring_1[1] + ring_2[1], player_hp)
                    if fight(Charecters):
                        if cost < min_cost:
                            min_cost = cost
                    else:
                        if cost > max_cost:
                            max_cost = cost
                    del Charecters["Player"]
    return min_cost, max_cost



# Returns true if the player wins the fight
def fight(Charecters):
    Charecters["Boss"] = parse("Day21/input.txt")
    while Charecters["Boss"].take_damage(Charecters["Player"].attack) > 0:
        if Charecters["Player"].take_damage(Charecters["Boss"].attack) <= 0:
            return False
    return True
...
least_cost, most_cost = Shop(Charecters)
print("Lowest Winning Cost:", least_cost, "Highest Losing Cost:", most_cost)
